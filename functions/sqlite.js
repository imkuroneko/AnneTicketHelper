...
module.exports = {
    isTicket: function(channel_id, guild) {
        const query = sql.prepare(" SELECT count(*) as count FROM tickets_details WHERE channel = ? AND guild = ? ");
    ...
    getCurTicketId: function(guild, channel) {
        const query = sql.prepare(" SELECT ticket FROM tickets_details WHERE guild = ? AND channel = ? ");
        return query.get(guild, channel).ticket.toString().padStart(5, '0');
    },

    getUserCreator: function(guild, channel) {
        const query = sql.prepare(" SELECT user FROM tickets_details WHERE guild = ? AND channel = ? ");
        return query.get(guild, channel).user.toString();
    },

    getTicketCategory: function(guild, channel) {
        const query = sql.prepare(" SELECT category FROM tickets_details WHERE guild = ? AND channel = ? ");
        return query.get(guild, channel).category.toString();
    },

    updateToOpen: function(guild, channel) {
        const query = sql.prepare(" UPDATE tickets_details SET status = 'A' WHERE guild = @gld AND channel = @chn; ");
        query.run({
            gld: guild,
            chn: channel
        });
    ...
    updateToClosed: function(guild, channel) {
        const query = sql.prepare(" UPDATE tickets_details SET status = 'C' WHERE guild = @gld AND channel = @chn; ");
        query.run({
            gld: guild,
            chn: channel
        });
    ...
    updateToDeleted: function(guild, channel) {
        ...
        const query = sql.prepare(" UPDATE tickets_details SET status = 'D', timestamp_deletion = @tms WHERE guild = @gld AND channel = @chn; ");
        query.run({
            gld: guild,
            chn: channel,
            tms: timestamp
